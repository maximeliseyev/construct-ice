...
mod tests {
    ...
    fn client_server_ntor_agree() {
        ...
        let server_result = server_ntor(
            &server_static,
            &server_epk,
            &client_epk.public,
        );

        assert_eq!(client_result.key_seed, server_result.key_seed, "KEY_SEED mismatch");
    ...
    fn different_server_gives_different_keys() {
        ...
        let r1 = client_ntor(&client_epk, &server1.public, &server1.node_id, &server_epk.public);
        let r2 = client_ntor(&client_epk, &server2.public, &server2.node_id, &server_epk.public);
    ...
    fn go_reference_ntor_vectors() {
        ...
        fn h20(s: &str) -> [u8; 20] {
            let mut out = [0u8; 20];
            for i in 0..20 { out[i] = u8::from_str_radix(&s[2*i..2*i+2], 16).unwrap(); }
            out
        }

        // Fixed test values from Go
        let client_priv = h("07102132435c6d7e8f98a9bacbe4f50617203142536c7d8e9fa8b9cadbf40516");
        let server_static_priv = h("a8abababababababababababababababababababababababababababababababab");
        ...
        let client_dirty_pub = MontgomeryPoint(h("276dca70899bf3dc12a90a25b42c77c8b43419d27361d452e565a2f514af547b"));
        let server_dirty_eph = MontgomeryPoint(h("78229a8773105ac4b8ea5266f340b3818fe94b46748a01eef68e4485e771344d"));
        // Server static pub (standard X25519)
        let server_static_pub = MontgomeryPoint::mul_base_clamped(server_static_priv);
        assert_eq!(hex::encode(server_static_pub.to_bytes()),
        ...
        assert_eq!(hex::encode(xy.to_bytes()),
            "3f275b8273197f1155410218a26a238df58bca173468ada4ba8ee9e43f473e74");
        assert_eq!(hex::encode(xb.to_bytes()),
        ...
        let result = compute_ntor(
            &xy.to_bytes(), &xb.to_bytes(),
            &node_id, &server_static_pub, &client_dirty_pub, &server_dirty_eph,
        );

        // Expected from Go
        assert_eq!(hex::encode(result.key_seed),
            "86ff4ea92c7c913fd2fa3df39c4153175ea8060e94df06fd6fa0fff66de6376e",
            "KEY_SEED mismatch with Go reference");
        assert_eq!(hex::encode(result.auth),
